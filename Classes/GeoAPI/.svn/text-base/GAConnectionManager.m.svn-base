...
#else  // bad connection test 
NSString *const kGAQueryFormat = @"http://xxapi.xgeoapi.com/v1/q?apikey=%@&q=%@";
NSString *const kGASearchFormat = @"http://xxapi.xgeoapi.com/v1/search?apikey=%@&"
"lat=%f&lon=%f&radius=%dm&limit=%d";
NSString *const kGAUserViewFormat = @"http://xxapi.xgeoapi.com/v1/e/%@/userview/%@?apikey=%@";
NSString *const kGAListingFormat =@"http://xxapi.xgeoapi.com/v1/e/%@/view/listing?apikey=%@";
NSString *const kGAParentsFormat =@"http://xxapi.xgeoapi.com/v1/parents?apikey=%@&lat=%f&lon=%f";
NSString *const kGAPlaceParentsFormat =@"http://xxapi.xgeoapi.com/v1/e/%@/parents?apikey=%@";
#endif
...
- (void)requestStringFromView:(NSString *)viewName forPlace:(NSString *)guid 
{
  NSString *url = [NSString stringWithFormat:kGAUserViewFormat,
                   guid,
                   viewName,
...
{
  NSString *escapedQuery = [mqlQuery stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
  [self requestURL:[NSString stringWithFormat:kGAQueryFormat, apiKey_, escapedQuery]];
}
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
  ...
  DebugLog(@"Content-Type: %@", [httpResponse.allHeaderFields  objectForKey:@"Content-Type"]);
  DebugLog(@"Content-Length: %@", [httpResponse.allHeaderFields objectForKey:@"Content-Length"]);
#endif
}
